...
ans = seq_set[0]
cnt = 0
with open("ans/rans.txt", "w", encoding='utf-8') as f:
    now = datetime.now()
...
while True:
    ...
    for step, s_set in enumerate(seq_set):
        ...
        for ind, val in enumerate(s_set):
            train_data[step][ind] = blo62((base_list[ind], val)) + 4
